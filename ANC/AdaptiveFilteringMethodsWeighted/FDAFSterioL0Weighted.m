function [ error, MSerror, timeOfConvergence,timeOfComputation,  w1, w2, filters1, filters2,weights1, weights2,variances1, variances2] = FDAFSterioL0Weighted( referenceSignal1, referenceSignal2, filtredSignal, filterSize,  convergenceThreshold, mu_k1, mu_k2, interval1, interval2, upperLimit, memoryInSeconds )
% This function implements the L0 weighted Sterio version of FDAF algorithm.
...
%             mu = alpha*(1/max(P_k)) ;
            ...
            % Append a zero block 
             ...
             gradient2 = g*ifft(mu_k2.*GRADIENT2);
              ...
              W1 = W1 +  2*fft(gradient1) + fft(k1.*ExponentialAprrox(beta,w1)) ; 
              W2 = W2 +  2*fft(gradient2) + fft(k2.*ExponentialAprrox(beta,w2)) ; 
