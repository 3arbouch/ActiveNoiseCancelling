...
experimentName = 'experiment30.mat';
...
% Generate the white noise signal   
 ...
 buffer = PsychPortAudio('CreateBuffer', [], [noise;silence]);
...
%% Initialization of the device and the signals: Generate ans Save signals for estimation of P(z)
 ...
 silence = zeros(1,length(noise));
 buffer = PsychPortAudio('CreateBuffer', [], [silence;noise]);
...
referenceMicPrime = completeSignal(1,:);
errorMicPrime = completeSignal(2,:);
